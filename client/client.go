...
type Client struct {
	...
	clients      map[uint64]kvpb.RaftKVClient
...
func New(clusterID uint64, nodeAddrs map[uint64]string) *Client {
	...
	return &Client{
		...
		clients:      make(map[uint64]kvpb.RaftKVClient),
...
func (c *Client) Close() error {
	...
	c.clients = make(map[uint64]kvpb.RaftKVClient)
	c.mu.Unlock()
...
func (c *Client) Propose(ctx context.Context, req *kvpb.RaftCmdRequest) (*kvpb.RaftCmdResponse, error) {
	if req == nil {
	...
	var lastResp *kvpb.RaftCmdResponse
	var lastErr error
...
func (c *Client) proposeToNode(ctx context.Context, nodeID uint64, req *kvpb.RaftCmdRequest) (*kvpb.RaftCmdResponse, error) {
...
func (c *Client) clientForNode(nodeID uint64) (kvpb.RaftKVClient, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) cachedRegionForRequest(req *kvpb.RaftCmdRequest) uint64 {
...
func (c *Client) rememberHeader(header *kvpb.ResponseHeader) uint64 {
...
func retryableHeader(header *kvpb.ResponseHeader) bool {
	...
	return strings.Contains(strings.ToLower(header.Error), "not leader")
}

func requestKey(req *kvpb.RaftCmdRequest) ([]byte, error) {
...
func cloneRequest(req *kvpb.RaftCmdRequest) *kvpb.RaftCmdRequest {
	out := &kvpb.RaftCmdRequest{}
	if req.Header != nil {
		out.Header = &kvpb.RequestHeader{
			ClusterId: req.Header.GetClusterId(),
			NodeId:    req.Header.GetNodeId(),
		}
	}

	out.Requests = make([]*kvpb.Request, len(req.Requests))
	for i, r := range req.Requests {
		if r == nil {
			continue
		}

		cloned := &kvpb.Request{CmdType: r.CmdType}
		if r.Get != nil {
			cloned.Get = &kvpb.GetRequest{Cf: r.Get.Cf, Key: cloneBytes(r.Get.Key)}
		}
		if r.Put != nil {
			cloned.Put = &kvpb.PutRequest{Cf: r.Put.Cf, Key: cloneBytes(r.Put.Key), Value: cloneBytes(r.Put.Value)}
		}
		if r.Delete != nil {
			cloned.Delete = &kvpb.DeleteRequest{Cf: r.Delete.Cf, Key: cloneBytes(r.Delete.Key)}
		}
		if r.Scan != nil {
			cloned.Scan = &kvpb.ScanRequest{Cf: r.Scan.Cf, StartKey: cloneBytes(r.Scan.StartKey), Limit: r.Scan.Limit}
		}
